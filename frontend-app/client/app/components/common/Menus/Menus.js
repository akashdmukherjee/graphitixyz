...
class Menus extends React.Component {
  ...
  onDOMClick(event) {
    if (
      (this.element !== event.target && !this.element.contains(event.target)) ||
      event.target.className.match(/Menu-link/i)
    ) {
      this.onClickedAway();
    }
  }
  ...
  render() {
    ...
    return (
      <div className={`Menus-wrapper ${className}`} style={wrapperStyle}>
        <WrappedComponent
        ...
        >
          <SubMenus list={list} style={style} className={className}>
            {children}
          </SubMenus>
        </div>
      </div>
...
Menus.propTypes = {
  ...
  menusWrapperStyle: React.PropTypes.object,
