...
window.get_chartContainerDimensions = function() {
  let chart_container_width = document.getElementById("chart_container")
    .offsetWidth;
  let chart_container_height = document.getElementById("chart_container")
    .offsetHeight;

  return [chart_container_width, chart_container_height];
};

window.calculateChartArea = function() {
  let chart_area = {
    width: 0,
    height: 0
  };
  ...
  if (horizontal_scroll == "scroll") {
    ...
    width =
      chart_area["width"] +
      space_for_axes.left +
      space_for_axes.right +
      2 * padding;
  } else {
    width = container_width;

    chart_area["width"] =
      width - space_for_axes.left - space_for_axes.right - 2 * padding;
  ...
  if (vertical_scroll == "scroll") {
    ...
    height =
      chart_area["height"] +
      space_for_axes.top +
      space_for_axes.bottom +
      2 * padding;
  } else {
    height = container_height;

    chart_area["height"] =
      height - space_for_axes.top - space_for_axes.bottom - 2 * padding;
  ...
  x_axis_ranges.map(function(x_axis_range, x_axis_range_index) {
    let filtered_data = chart_data.filter(function(datarecord) {
      for (let i = 0; i < x_axis_range.fields.length; i++) {
        if (datarecord.x_metric == x_axis_range.fields[i].field_name)
          return datarecord;
      }
    });
  ...
  y_axis_ranges.map(function(y_axis_range, y_axis_range_index) {
    let filtered_data = chart_data.filter(function(datarecord) {
      for (let i = 0; i < y_axis_range.fields.length; i++) {
        if (datarecord.y_metric == y_axis_range.fields[i].field_name)
          return datarecord;
      }
    });
...
window.drawSvgContainer = function() {
  let svg = d3
    .select("#" + render_div)
    .append("svg")
    .attr("height", 10)
...
window.sizeAndPositionSvgContainer = function() {
  svg
    .attr(
      "height",
      chart_area["height"] + space_for_axes.top + space_for_axes.bottom
    )
    .attr(
      "width",
      chart_area["width"] + space_for_axes.left + space_for_axes.right
    );
};

window.getLabels_fromUqkeys = function(uqkeys) {
  let _labels = [];
  let num_levels = 0;
  ...
  _labels.map(function(_label, _label_index) {
    ...
    for (let i = _label_index; i < _labels.length; i++) {
      if (_label.level == _labels[i].level && _label.val == _labels[i].val) {
        ...
        _label.level == _labels[i].level && _label.val != _labels[i].val
      ) {
        break;
      }
    }
  ...
  _labels.map(function(label, label_index) {
    ...
    for (let i = 0; i < label_index; i++) {
      ...
      for (let l = 1; l <= label.level; l++) {
        condition =
          condition &&
          _labels[label_index + (l - 1)].val == _labels[i + (l - 1)].val;
      }
...
window.createXAxesLabels = function(svg) {
  ...
  if (is_groupers_x) {
    let x_labels = getLabels_fromUqkeys(x_uqkeys);
...
window.positionXAxesLabels = function() {
  x_uqkeys.map(function(x_uqkey, x_uqkey_index) {
    d3
  ...
  d3
    ...
    .attr("x", function(d, i) {
      return (
        chart_area["width_group"] * d.offset +
        chart_area["width_group"] * d.occurrences / 2
      );
    })
    .attr("y", function(d, i) {
...
window.createYAxesLabels = function(svg) {
  ...
  if (is_groupers_y) {
    let y_labels = getLabels_fromUqkeys(y_uqkeys);
...
window.positionYAxesLabels = function() {
  // REFER X LABELS
  y_uqkeys.map(function(y_uqkey, y_uqkey_index) {
    d3
  ...
  __selection
    ...
    .attr("y", function(d, i) {
      return (
        chart_area["height_group"] * d.offset +
        chart_area["height_group"] * d.occurrences / 2
      );
    });
};

window.createAxesGridlines = function(svg) {
  ...
  y_uqkeys.map(function(y_uqkey, y_uqkey_index) {
    ...
    x_gridlines.map(function(x_gridline) {
      svg
        ...
        .attr(
          "y1",
          yScales[scalenum_for_this_axis](x_gridline) +
            chart_area["height_group"] * y_uqkey_index
        )
  ...
  x_uqkeys.map(function(x_uqkey, x_uqkey_index) {
    svg
      ...
      .attr(
        "x1",
        space_for_axes.left + chart_area["width_group"] * (x_uqkey_index + 1)
      )
      .attr("y1", 0)
      .attr(
        "x2",
...
window.x_calc1 = function(datarecord, chart_area, xScales) {
...
window.x_calc2 = function(datarecord) {
  let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
  let w = chart_area["width_group"] / n;
...
window.y_calc1 = function(datarecord, chart_area, yScales) {
  let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
...
window.y_calc2 = function(datarecord, chart_area, yScales) {
  let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
  let h = chart_area["height_group"] / n;
...
window.y_calc3 = function(datarecord, chart_area, yScales) {
  let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
  let h = chart_area["height_group"] / n;
...
window.getBarThickness = function(datarecord, thickness_multiplier) {
  let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
  let w = chart_area["width_group"] / n;
  let h = chart_area["height_group"] / n;
...
window.getBackgroundColor = function(datarecord) {
...
window.createInteractions = function() {
  ...
  datapoints.on("mouseover", function(d) {
    ...
    let bar_thickness = getBarThickness(d, thickness_multiplier);
    ...
    if (
      (is_metrics_nox_noy && !is_metrics_angle) ||
      is_metrics_nox_y ||
      is_metrics_x_noy
    ) {
      ...
      .html(function() {
        ...
        if (is_fragments) {
          fragments.map(function(fragment, fragment_index) {
            ...
            tooltip_text +=
              capitalizeFirstLetter(fragment.field_name) +
              ": " +
              fragment_vals[fragment_index];
          });
        }
        ...
        if (is_groupers_x) {
          ...
          x_groupers.map(function(x_grouper, x_grouper_index) {
            ...
            tooltip_text +=
              capitalizeFirstLetter(x_grouper.field_name) +
              ": " +
              x_grouper_vals[x_grouper_index];
          });
        }
        ...
        if (is_groupers_y) {
          ...
          y_groupers.map(function(y_grouper, y_grouper_index) {
            ...
            tooltip_text +=
              capitalizeFirstLetter(y_grouper.field_name) +
              ": " +
              y_grouper_vals[y_grouper_index];
          });
        }
        ...
        if (
          ...
          tooltip_text +=
            "<div style='padding: 10px; border-top: 1px solid #f3c162;'>";
        }

        if (is_metrics_x) {
          tooltip_text +=
            capitalizeFirstLetter(d.x_metric) + ": " + d.x_metric_val;
        }

        if (is_metrics_y) {
          if (is_metrics_x) tooltip_text += "<br/>";
          tooltip_text +=
            capitalizeFirstLetter(d.y_metric) + ": " + d.y_metric_val;
        }

        if (is_metrics_subject) {
          if (is_metrics_x || is_metrics_y) tooltip_text += "<br/>";
          tooltip_text +=
            capitalizeFirstLetter(d.subject_metric) +
            ": " +
            d.subject_metric_val;
        }

        if (is_metrics_size) {
          if (is_metrics_x || is_metrics_y || is_metrics_subject)
            tooltip_text += "<br/>";
          tooltip_text +=
            capitalizeFirstLetter(d.size_metric) + ": " + d.size_metric_val;
        }
        ...
        if (is_metrics_color) {
          if (
            ...
            tooltip_text += "<br/>";
          tooltip_text +=
            capitalizeFirstLetter(d.color_metric) + ": " + d.color_metric_val;
        }
        ...
        if (is_metrics_angle) {
          if (
            ...
            tooltip_text += "<br/>";
          tooltip_text +=
            capitalizeFirstLetter(d.angle_metric) + ": " + d.angle_metric_val;
        }
...
window.createTextLabels = function(svg) {
  return svg
    .append("g")
    ...
    .attr("x", function(datarecord) {
      if (is_metrics_x_noy) {
        // BAR CHART
        return space_for_axes.left + x_calc1(datarecord, chart_area, xScales);
      } else if (is_metrics_nox_y) {
        // BAR CHART
        return space_for_axes.left + x_calc2(datarecord);
      } else if (is_metrics_nox_noy) {
        // MATRIX CHART
        let x_pos = 0;
        ...
        // SCATTER CHART
        return x_calc1(datarecord, chart_area, xScales);
      } else return 0;
    })
    .attr("y", function(datarecord) {
      if (is_metrics_x_noy) return y_calc2(datarecord, chart_area, yScales);
      else if (is_metrics_nox_y)
        return y_calc1(datarecord, chart_area, yScales);
      else if (is_metrics_nox_noy) {
        let y_pos = 0;
        let n = datarecord.fragment_uqkey_maxindex; // n is length of fragment_uqkeys
        let h = chart_area["height_group"] / n;
    ...
    .attr("fill", function(datarecord) {
      ...
      if (is_metrics_x_y) {
        ...
        let background_color = getBackgroundColor(datarecord);
