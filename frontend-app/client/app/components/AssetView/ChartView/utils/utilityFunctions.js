window.search_datarecord_inDataset = function (dataset, field_name, field_val) {
...
window.get_distinctVals_ofField_fromDataset = function (dataset, field_name) {
  let unique = {};
  let distinct = [];
...
window.isObjectEqual = function (a, b) {
  let prop;
  for (prop in a) {
...
window.sort_dataset = function (dataset, sorting_field) {
...
window.deduplicate_dataset = function (dataset) {
  let isAdded,
    arr = [];
...
window.parse_uqkey = function (uqkey) {
  // UQKEY STRUCTURE:
  // groupers ~~~~ metric
...
window.parse_uqkey_nometric = function (uqkey) {
  // UQKEY STRUCTURE:
...
window.addScaleUnits_onLargeNumbers = function (number) {
...
window.getFontColor_forBackgroundRgb = function (rgb) {
