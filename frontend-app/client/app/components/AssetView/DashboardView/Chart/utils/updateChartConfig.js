const get_chartContainerDimensions = function (chartContainerId) {
  let chart_container_width = document.getElementById(chartContainerId).offsetWidth;
  let chart_container_height = document.getElementById(chartContainerId).offsetHeight;

  return [chart_container_width, chart_container_height];
};

const updateChartConfig = function (vizData, v, colorcodes_dictionary, chartContainerId) {
  const chartConfig = {};
  ...
  chartConfig.container_width = get_chartContainerDimensions(chartContainerId)[0];
  chartConfig.container_height = get_chartContainerDimensions(chartContainerId)[1];
  chartConfig.render_div = chartContainerId;
  ...
  chartConfig.bar_thickness = window.bar_thickness || 0.9;
  chartConfig.hole_size = window.hole_size || 0.8;
  chartConfig.is_stacked = true;
